...
suite('Extension Test Suite', () => {
  ...
  test('extension should register verilog.ctags.rebuildWorkspaceIndex command', async function () {
    this.timeout(10000);
    const extension = vscode.extensions.getExtension(EXTENSION_ID);
    assert.ok(extension, 'Extension should be present');
    ...
    const commands = await vscode.commands.getCommands(true);
    assert.ok(
      commands.includes('verilog.ctags.rebuildWorkspaceIndex'),
      'verilog.ctags.rebuildWorkspaceIndex command should be registered'
    );
  });
});
