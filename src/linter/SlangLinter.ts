...
const convertToWslPath = (inputPath: string) => {
  const cmd: string = `wsl wslpath '${inputPath}'`;
...
export default class SlangLinter extends BaseLinter {
  ...
  private updateConfig() {
    ...
    const p = <string[]> this.configuration.get('includePath');
    this.includePath = p.map((includePath: string) => this.resolvePath(includePath));
  ...
  protected lint(doc: vscode.TextDocument) {
    ...
    if (isWindows) {
      if (this.useWSL) {
        docUri = convertToWslPath(docUri);
        this.logger.info(`Rewrote docUri to ${docUri} for WSL`);

        docFolder = convertToWslPath(docFolder);
