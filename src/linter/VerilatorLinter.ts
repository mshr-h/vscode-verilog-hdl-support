...
export default class VerilatorLinter extends BaseLinter {
  ...
  private getConfig() {
    ...
    this.verilatorArgs = <string>(
      vscode.workspace.getConfiguration().get('verilog.linting.verilator.arguments', '')
    );
    this.runAtFileLocation = <boolean>(
      vscode.workspace.getConfiguration().get('verilog.linting.verilator.runAtFileLocation')
    );
    this.useWSL = <boolean>(
  ...
  protected lint(doc: vscode.TextDocument) {
    ...
    if (isWindows) {
      if (this.useWSL == true) {
        ...
        docUri = child.execSync(docUriCmd, {}).toString().replace(/\r?\n/g, '');
        this.logger.info(`Rewrote docUri to ${docUri} for WSL`);

        let docFolderCmd: string = `wsl wslpath '${docFolder}'`;
        docFolder = child.execSync(docFolderCmd, {}).toString().replace(/\r?\n/g, '');
