...
export {
  ...
  type BuildModelsimArgsOptions,
} from './genericLintParsers';

function getSpec(): LinterExecutionSpec {
  const spec = getLinterSpec('modelsim');
  if (!spec) {
    throw new Error('Missing modelsim linter spec');
  }
  return spec;
}

export default class ModelsimLinter extends GenericExternalLinter {
