...
class TemporaryFile {
  ...
  constructor(prefix: string, ext: string) {
    this.path = path.join(os.tmpdir(), prefix + "-" + crypto.randomBytes(16).toString('hex') + ".tmp" + ext);
  }
...
export class VerilogFormatProvider implements vscode.DocumentFormattingEditProvider {
  ...
  provideDocumentFormattingEdits(
    ...
    let iStyleVerilogFormatter = new IStyleVerilogFormatterEditProvider("iStyleVerilogFormatter", ".v", this.logger);
    let veribleVerilogFormatter = new VeribleVerilogFormatEditProvider("veribleVerilogFormatter", ".v", this.logger);
...
export class SystemVerilogFormatProvider implements vscode.DocumentFormattingEditProvider {
  ...
  provideDocumentFormattingEdits(
    ...
    let formatter: string | null = <string>(settings.get("formatter", null));
    let veribleVerilogFormatter = new VeribleVerilogFormatEditProvider("veribleVerilogFormatter", ".sv", this.logger);
